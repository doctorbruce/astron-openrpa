...
contextBridge.exposeInMainWorld('opencodeApi', {
  ...
  getSession: (sessionId: string, options?: { includeWorkspace?: boolean }) =>
    ipcRenderer.invoke('opencode:getSession', { sessionId, includeWorkspace: options?.includeWorkspace }),
  ...
  abortSession: (sessionId: string) => ipcRenderer.invoke('opencode:abortSession', sessionId),
  sendMessage: (payload: {
    sessionID: string
    text: string
    system?: string | null
  ...
  pickWorkspace: (currentPath?: string | null) => ipcRenderer.invoke('opencode:pickWorkspace', currentPath),
